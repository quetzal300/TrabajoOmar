...
@Component({
  selector: 'app-publicidad-c',
  templateUrl: './publicidad-c.component.html',
  styleUrls: ['./publicidad-c.component.css']
})
export class PublicidadCComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.llamar.getPublicasionesC().subscribe(respuesta =>{
  ...
  Eliimnar(id){
    console.log(id);
    this.llamar.elimiarC(id).subscribe(
      res =>{
      console.log(res)
    }
    );
