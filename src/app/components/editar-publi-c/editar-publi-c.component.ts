...
@Component({
  selector: 'app-editar-publi-c',
  templateUrl: './editar-publi-c.component.html',
  styleUrls: ['./editar-publi-c.component.css']
})
export class EditarPubliCComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.Publisidad = new Publicasiones();
    this.Publisidad.url = localStorage.getItem('url');
    this.Publisidad.Nombre = localStorage.getItem('Nombre');
    this.Publisidad.Duracion = localStorage.getItem('Duracion');
    this.Publisidad.Creado = localStorage.getItem('Creado');
    this.Publisidad.Sesion = parseInt(localStorage.getItem('Sesion'));
    this.Publisidad.id = localStorage.getItem('id');
    this.Publisidad.Categoria = localStorage.getItem('Categoria');
  ...
  guardar(formu: NgForm,archivo:FileItem[]){
    ...
    if(formu.invalid){
      ...
      if(archivo.length!==0){
        console.log('1 Formulario echo ')
        this.cargaImg.ActualizaImagenAFirebeC(archivo,this.Publisidad,localStorage.getItem('id'));

        console.log(this.Publisidad);
      }else{


        this.publicidad.actualizarC(this.Publisidad,localStorage.getItem('id')).subscribe(respuesta =>{
          console.log(respuesta)
          });
          console.log(archivo)
          console.log('2 Formulario echo ')
          console.log(this.Publisidad);
      }
    }

      //this.cargaImagen(archivo,this.Publisidad);
     // this.cargaImg.ActualizaImagenAFirebe(archivo,this.Publisidad,localStorage.getItem('id'));
  ...
  compruevaFecha(){
    ...
    console.log(`${ddH}/${mmH}/${aaaaH}`)
     ...
     if(anoI+2000 < aaaaH){
       ...
       console.log('qui la fecha tiene que ser mayor a la de hoy');
       return false;
     }else{
